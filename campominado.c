...
void inicializar_campo(int **bombas, char **jogo, int linhas, int colunas, int num_bombas) {
    ...
    while (bombas_colocadas < num_bombas) {
        ...
        if (bombas[rand_linha][rand_coluna] != -1) {  // Só coloca a bomba se o lugar estiver livre
    ...
    while (fim != 1) {
        ...
        if (sscanf(entrada, "%d,%d", &x, &y) != 2) {
            ...
            if (x < 1 || x > linhas || y < 1 || y > colunas) {
                ...
                if (jogo[x][y] != '.') {  // Se o jogador já escolheu essa célula
                    printf("Coordenadas já escolhidas! Tente novamente.\n\n");
                } else {
                    if (bombas[x][y] == 0) {  // Se a célula for zero, faz o flooding
                        revelados += espalhamento(bombas, jogo, linhas, colunas, x, y);
                    } else if (bombas[x][y] != -1) {      // Se for segura, apenas revela
                    ...
                    if (revelados == total_sem_bomba) {  // Se revelou todas as células seguras
